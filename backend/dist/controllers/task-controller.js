...
exports.createTaskController = (0, async_handler_1.asyncHandler)(async (req, res) => {
...
exports.updateTaskController = (0, async_handler_1.asyncHandler)(async (req, res) => {
    const userId = req.user?._id;
...
exports.getAllTasksController = (0, async_handler_1.asyncHandler)(async (req, res) => {
    const userId = req.user?._id;
    const workspaceId = workspace_validation_1.workspaceIdSchema.parse(req.params.workspaceId);
    const filters = {
...
exports.getTaskByIdController = (0, async_handler_1.asyncHandler)(async (req, res) => {
    const userId = req.user?._id;
    const taskId = task_validation_1.taskIdSchema.parse(req.params.id);
    const projectId = project_validation_1.projectIdSchema.parse(req.params.projectId);
    const workspaceId = workspace_validation_1.workspaceIdSchema.parse(req.params.workspaceId);
...
exports.deleteTaskController = (0, async_handler_1.asyncHandler)(async (req, res) => {
    const userId = req.user?._id;
    const taskId = task_validation_1.taskIdSchema.parse(req.params.id);
    const workspaceId = workspace_validation_1.workspaceIdSchema.parse(req.params.workspaceId);
    const { role } = await (0, member_service_1.getMemberRoleInWorkspace)(userId, workspaceId);
